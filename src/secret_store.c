 ...
 */
static int load_or_generate_salt(const char *store_dir, unsigned char *salt, size_t salt_size)
{
    char salt_path[512];
    snprintf(salt_path, sizeof(salt_path), "%s/.salt", store_dir);
...
static int derive_key(secret_store_t *store)
{
    ...
    /* Combine machine-id with optional config salt */
    char combined[256];
    int combined_len;
     ...
     * The salt is stored in the secrets directory and generated once.
     */
    unsigned char pbkdf_salt[SALT_SIZE];
    if (load_or_generate_salt(store->config.store_dir, pbkdf_salt, SALT_SIZE) != 0) {
        snprintf(store->error_buf, sizeof(store->error_buf),
                 "Failed to load or generate PBKDF2 salt");

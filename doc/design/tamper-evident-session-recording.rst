...
- **Resource bounds:** header ≤ 8 KiB with a 30 s header timeout; per-session byte cap; total duration cap; the peer's liveness instead of an idle timeout (§2). The ``Accept=yes`` model gives one process per connection, but because a recorded connection now lives for the whole session, ``ob-record.socket`` sets ``MaxConnections`` above systemd's default of 64 and ``MaxConnectionsPerSource`` (per ``SO_PEERCRED`` uid), so one local user cannot hold the world-connectable socket's slots open and lock everyone out. (DoS is explicitly *out* of the security-review exclusions, but bounding it is good hygiene.)

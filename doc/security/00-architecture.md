...
| `pamAccessServerGroups`      | —       | (Optionnel) table `client_id → groupe` qui **impose** le `server_group` à `/pam/authorize`. Inadaptée quand un `client_id` couvre un projet **multi-groupes** → laisser vide. `/pam/bastion-cert` n'utilise **aucun** groupe : il ne s'appuie que sur le voucher |

...
Open Bastion can integrate with `CrowdSec <https://www.crowdsec.net/>`__ for enhanced security:
...
1. A running CrowdSec Local API (LAPI) - either locally or via `Crowdsieve <https://github.com/linagora/crowdsieve>`__
...
`Crowdsieve <https://github.com/linagora/crowdsieve>`__ is a filtering proxy between local CrowdSec instances and the Central API (CAPI). It provides:
...
The ``crowdsec_whitelist`` option allows you to specify IPs and networks that should bypass CrowdSec checks entirely. This is useful for:
...
See :ref:`Security Analysis <crowdsec-security-considerations-1>` for the DoS prevention use case.

.. _crowdsec-security-considerations-1:
...
- :doc:`Security Features </security>` - Other security features

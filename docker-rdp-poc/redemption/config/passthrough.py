...
try:
    ...
    # A missing/unwritable log directory must not abort authentication.
    pass
...
class LLNGAuthenticator:
    ...
    def authorize(self, user: str, target_host: str, client_ip: str) -> dict:
        """Uses the existing /pam/authorize endpoint with service=rdp, so the
        same pamAccessServerGroups apply to RDP as to SSH."""
...
def read_input() -> dict:
    try:
        ...
        # Do not log input_data: it contains passwords.
...
def write_output(result: dict) -> None:
    # Do not log result: it may contain passwords.
...
def main():
    ...
    if auth_result.get('authorized'):
        # PoC: passes through the credentials the user supplied; production
        # deployments should source target credentials from a vault instead.
